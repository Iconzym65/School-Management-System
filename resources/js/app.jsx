...
function AdminPortal() {
    ...
    const loadAllData = async () => {
        ...
        try {
            const [teachersRes, studentsRes, coursesRes, timetablesRes, departmentsRes, yearsRes, semestersRes] = await Promise.all([
            ...
            const teacherData = toArray(teachersRes.data);
            const studentData = toArray(studentsRes.data);
            const courseData = toArray(coursesRes.data);
            ...
            if (!timetableForm.course_id && courseData.length) {
                setTimetableForm((prev) => ({ ...prev, course_id: courseData[0].id }));
            }
            if (!timetableForm.teacher_id && teacherData.length) {
                setTimetableForm((prev) => ({ ...prev, teacher_id: teacherData[0].id }));
            }
        } catch (error) {
